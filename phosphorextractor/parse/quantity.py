...
def value_element(units=(OneOrMore(T('nm')) |OneOrMore(W('nm'))|OneOrMore(R('(cm−1|cm-1)'))|OneOrMore(I('cm')+R('[--—\-‒–—―−]\d?'))|OneOrMore(W('cm')+R('(-|—)1'))| OneOrMore(T('NNP')) | OneOrMore(T('NNPS')) | OneOrMore(T('NNS')))('raw_units').add_action(merge)):
    """
...
def value_element_plain():
    ...
    value = Optional(lbrct).hide() + (value_range | value_div | value_single)('raw_value') + Optional(rbrct).hide()
    return value
def peak_value_element_plain():
    delim = R('[,:;\./]$') | W(' ') 
    
    units = (W('nm'))('units').add_action(merge)
    
    value_1 = (R('^\d+(\.\,\d+)?$'))('value_1').add_action(join)
    
    multipeaks = (value_1 + ZeroOrMore((I('and')|delim | hyphen| I('to')) + value_1) )
    value = (value_1 + ZeroOrMore((delim | hyphen| I('to')) + value))('raw_value').add_action(join)
