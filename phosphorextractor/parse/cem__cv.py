...
doped_ion = (colon
             ...
             +ZeroOrMore(I('x'))
             +ZeroOrMore(I('y'))
             +OneOrMore((ion_symbol|transition_metals+ion_label|lanthanides)+ZeroOrMore(R('\\\\')))
             )
phosphors = (
    I('Ca19Mn2(PO4)14')|I('(Sr0.5Eu0.5)Si9Al19ON31')|I('Ca–α-sialon')|I('Sr–α-SiAlON:Eu2+')|I('(Sr0.5Eu0.5)Si9Al19ON31')|I('β-sialon:Eu2+')|I('α-sialon:Eu2+')|I('γ-sialon:Eu2+')|I('β-sialon')|I('α-sialon')|I('γ-sialon')
    | I('Sr1-xEuxSi9Al19ON31') | I('Ba3ScB3O9:Eu2+') | I('CaBa[Li2Al6N8]') | I('Ca2MgSi2O7') |I('Zn2SiO4:Mn2+')|I('Ca/Eu–α-SiAlONs')|I('Li-α-SiAlON:Eu2+')|R('^(Ca|Ba|Sr|Na|K|Rb|Y|Zn|La|Mg|Al|Li|Gd|Cs|Co|Ga|As)?-?(α|γ|β)?-(SiAlON|sialon)s?:?(Eu|Ce|Mn|Tb|Cr)?\d?\+?')
    |R('α-SrNCN:Eu2+')|ZeroOrMore(R('(Ca|Ba|Sr|Na|K|Rb|Y|Zn|La|Mg|Al|Li|Gd|Cs|Co|Ga|As)')+ hyphen) +ZeroOrMore(R('(α|γ|β)')+hyphen)+ R('(SiAlON|sialon)s?')+ZeroOrMore(R(':')+R('(Eu|Ce|Mn|Tb|Cr)?\d?\+?'))
)

name_with_doped_label = ((chemical_name + OneOrMore(colon|delim|hyphen| I('with') | I('for') | I('doped')|I('activated')) + (ion_symbol|lanthanides))
                         |((ion_symbol|transition_metals|lanthanides) + ZeroOrMore(hyphen|delim)+OneOrMore(I('doped')|I('codoped')|I('activated')|I('with')|I('in')+I('the')|I('in'))+chemical_name)
                         |(chemical_name + doped_ion)|(ion_symbol|transition_metals)+R('[,\/-]?(and)?')+(ion_symbol|transition_metals)+R('[,\/-]?')+R('co\-?doped')+chemical_name
                         |phosphors
                         )('compound').add_action(join).add_action(fix_whitespace)
