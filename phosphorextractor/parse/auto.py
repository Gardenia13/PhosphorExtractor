...
from .cem import cem, chemical_label, lenient_chemical_label,phosphor
from .actions import merge, join
from .elements import W, I, R, T, Optional, Any, OneOrMore, Not, ZeroOrMore, Group, SkipTo, Or
from ..utils import first
from .quantity import magnitudes_dict, value_element, extract_units, value_element_plain,peak_value_element_plain, value_element_with_exp, value_element_plain_with_exponent, lbrct, rbrct
...
disallowed_perovskites = ['PbI2', 'SnO2']
...
class AutoSentenceParser(BaseAutoParser, BaseSentenceParser):
    ...
    def __init__(self, lenient=False, chem_name=(phosphor|cem | chemical_label | lenient_chemical_label)):
    ...
    def root(self):
        ...
        if hasattr(self.model, 'dimensions') and not self.model.dimensions:
            ...
            value_phrase = peak_value_element_plain()
            ...
            specifier = self.model.specifier.parse_expression('specifier')
            if self.lenient:
                value_phrase = (value_element(unit_element) | peak_value_element_plain())
            else:
