...
class Cleaner(object):
    ...
    def __call__(self, doc):
        ...
        url_pattern1 = r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+'
        url_pattern2 = r'https: // [ ^\s]+'
        url_pattern3 = r'"object": \[.*?"@mimetype": "image/gif"}]'
        url_pattern4 = r'Acknowledgment References.*?Elsevier B.V'
        url_pattern5 = r'All rights reserved..*?Elsevier B.V.'
        url_pattern6 = r'References.*?"scopus-id":'
        json_pattern = r'\[img[^\]]*\]'
        if self.give_up:
            for el in doc.iter():
                if el.text is not None:
                    el.text = re.sub(url_pattern1, '', el.text)
                    el.text = re.sub(url_pattern2, '', el.text)
                    el.text = re.sub(url_pattern3, '', el.text)
                    el.text = re.sub(url_pattern4, '', el.text)
                    el.text = re.sub(url_pattern5, '', el.text)
                    el.text = re.sub(url_pattern6, '', el.text)
                    el.text = re.sub(json_pattern, '', el.text)
